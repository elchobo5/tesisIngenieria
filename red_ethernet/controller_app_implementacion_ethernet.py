...
class GUIServerApp(app_manager.RyuApp):
	...
	self.origins = {}
	self.tcpportorigins = {}
	self.transportorigins = {}
	self.surrogates = {}
	self.tcpportsurrogates = {}
	...
	self.mac_to_port = {}
	self.ip_to_port = {}
	self.rt_surrogate_origin = {}


	macs_borders = db.borderSwitches
	for mac_border_aux in macs_borders.find():
		for mac_border in mac_border_aux['switches']:
			self.mac_ce_borders[mac_border['dpid']] = mac_border['mac']
    ...
    def switch_features_handler(self, ev):
        ...
        #Se agrega flujo, de que todo lo que vaya para algun puerto donde esta escuchando algun origen de tcp vaya al controlador
	for key in self.all_tcpports:
		for key2 in self.all_tcpports[key]:
			if (key2 == "tcp"):
				match = parser.OFPMatch(eth_type=ether.ETH_TYPE_IP,tcp_dst=key,ip_proto=6)
				actions = [parser.OFPActionOutput(ofproto.OFPP_CONTROLLER,
						                  ofproto.OFPCML_NO_BUFFER)]
				self.add_flow(datapath, 1, match, actions)
				match2 = parser.OFPMatch(eth_type=ether.ETH_TYPE_IP,tcp_src=key,ip_proto=6)
				self.add_flow(datapath, 1, match2, actions)
			else:
				match = parser.OFPMatch(eth_type=ether.ETH_TYPE_IP,udp_dst=key,ip_proto=17)
				actions = [parser.OFPActionOutput(ofproto.OFPP_CONTROLLER,
						                  ofproto.OFPCML_NO_BUFFER)]
				self.add_flow(datapath, 1, match, actions)
				match2 = parser.OFPMatch(eth_type=ether.ETH_TYPE_IP,udp_src=key,ip_proto=17)
				self.add_flow(datapath, 1, match2, actions)
	...
	self.mac_to_port[dpid][mac_src] = in_port
		...
		if (dpid in self.mac_ce_borders and mac_src == self.mac_ce_borders[dpid] and ip_src in self.ip_to_port[dpid] and self.ip_to_port[dpid][ip_src] != in_port):	
			if (udp_pkt):
				#descartar paquete
			...
			#self.logger.info("ofproto_v1_3.OFPP_CONTROLLER %d",ofproto_v1_3.OFPP_CONTROLLER)
			if (tcp_pkt):
				if ((ip_dst in self.origins) and (tcp_dst in self.origins[ip_dst]) and ('tcp' in self.origins[ip_dst][tcp_dst])):
					...
					if (len(self.origins[ip_dst][tcp_dst]['tcp']) > 0):
						if ((ip_dst in self.rt_surrogate_origin) and (tcp_dst in self.rt_surrogate_origin[ip_dst]) and ('tcp' in self.rt_surrogate_origin[ip_dst][tcp_dst])):
							actions = [parser.OFPActionSetField(tcp_dst=self.rt_surrogate_origin[ip_dst][tcp_dst]['tcp']['port']),parser.OFPActionSetField(ipv4_dst=self.rt_surrogate_origin[ip_dst][tcp_dst]['tcp']['surrogate']),parser.OFPActionOutput(out_port)]			
							self.logger.info("segunda vez ip_surrogate: %s, puerto_surrogate: %s",self.rt_surrogate_origin[ip_dst][tcp_dst]['tcp']['surrogate'],self.rt_surrogate_origin[ip_dst][tcp_dst]['tcp']['port'])									
						else:
							self.rt_surrogate_origin.setdefault(ip_dst, {})
							self.rt_surrogate_origin[ip_dst].setdefault(tcp_dst, {})
							self.rt_surrogate_origin[ip_dst][tcp_dst] = {}
							j = random.randint(0,len(self.origins[ip_dst][tcp_dst]['tcp'])-1)
							self.rt_surrogate_origin[ip_dst][tcp_dst]['tcp'] = {'port' : self.origins[ip_dst][tcp_dst]['tcp'][j]['port'], 'surrogate': self.origins[ip_dst][tcp_dst]['tcp'][j]['surrogate']}	
							actions = [parser.OFPActionSetField(tcp_dst=self.rt_surrogate_origin[ip_dst][tcp_dst]['tcp']['port']),parser.OFPActionSetField(ipv4_dst=self.rt_surrogate_origin[ip_dst][tcp_dst]['tcp']['surrogate']),parser.OFPActionOutput(out_port)]
							self.logger.info("Primera vez ip_surrogate: %s, puerto_surrogate: %s",self.rt_surrogate_origin[ip_dst][tcp_dst]['tcp']['surrogate'],self.rt_surrogate_origin[ip_dst][tcp_dst]['tcp']['port'])			
					else:
						if (dpid in self.mac_ce_borders):
							if (self.mac_ce_borders[dpid] == mac_src):
								actions = [parser.OFPActionOutput(out_port)]
							else:
								actions = [parser.OFPActionSetField(eth_dst=self.mac_ce_borders[dpid]),parser.OFPActionOutput(out_port)]
						else:
							actions = [parser.OFPActionOutput(out_port)]
					out = parser.OFPPacketOut(datapath=datapath,
								  buffer_id=ofproto.OFP_NO_BUFFER,
					...
					if (out_port != ofproto.OFPP_FLOOD):
						#agrego el flujo
						if (len(self.origins[ip_dst][tcp_dst]['tcp']) > 0):		
							del self.rt_surrogate_origin[ip_dst][tcp_dst]['tcp']
					...
					if (out_port != ofproto.OFPP_FLOOD):
						#agrego el flujo
				                self.add_flow(datapath, 100, match, actions, 30)
				elif ((ip_dst in self.surrogates and tcp_dst in self.surrogates[ip_dst] and 'tcp' in self.surrogates[ip_dst][tcp_dst]) or (ip_dst in self.origins and tcp_dst in self.origins[ip_dst] and 'tcp' in self.origins[ip_dst][tcp_dst])):
					...
					if (out_port != ofproto.OFPP_FLOOD):
						print "agrego el flujo"
					        #self.add_flow(datapath, 100, match, actions)
				elif ((ip_src in self.surrogates and tcp_src in self.surrogates[ip_src] and 'tcp' in self.surrogates[ip_src][tcp_src]) or (ip_src in self.origins and tcp_src in self.origins[ip_src] and 'tcp' in self.origins[ip_src][tcp_src])):
				...
				if ((ip_dst in self.origins) and (udp_dst in self.origins[ip_dst]) and ('udp' in self.origins[ip_dst][udp_dst])):
					# construct action list.
					match = parser.OFPMatch(eth_type=ether.ETH_TYPE_IP,udp_dst=udp_dst,ip_proto=ipv4_pkt.proto,ipv4_dst=ip_dst)
					if (len(self.origins[ip_dst][udp_dst]['udp']) > 0):
						if ((ip_dst in self.rt_surrogate_origin) and (udp_dst in self.rt_surrogate_origin[ip_dst]) and ('udp' in self.rt_surrogate_origin[ip_dst][udp_dst])):
							actions = [parser.OFPActionSetField(udp_dst=self.rt_surrogate_origin[ip_dst][udp_dst]['udp']['port']),parser.OFPActionSetField(ipv4_dst=self.rt_surrogate_origin[ip_dst][udp_dst]['udp']['surrogate']),parser.OFPActionOutput(out_port)]
						else:
							self.rt_surrogate_origin.setdefault(ip_dst, {})
							self.rt_surrogate_origin[ip_dst].setdefault(udp_dst, {})
							self.rt_surrogate_origin[ip_dst][udp_dst] = {}
							j = random.randint(0,len(self.origins[ip_dst][udp_dst]['udp'])-1)
							self.rt_surrogate_origin[ip_dst][udp_dst]['udp'] = {'port' : self.origins[ip_dst][udp_dst]['udp'][j]['port'], 'surrogate': self.origins[ip_dst][udp_dst]['udp'][j]['surrogate']}	
							#actions = [parser.OFPActionSetField(udp_dst=self.origins[ip_dst][udp_dst]['udp'][0]['port']),parser.OFPActionSetField(ipv4_dst=self.origins[ip_dst][udp_dst]['udp'][0]['surrogate']),parser.OFPActionOutput(out_port)]
							actions = [parser.OFPActionSetField(udp_dst=self.rt_surrogate_origin[ip_dst][udp_dst]['udp']['port']),parser.OFPActionSetField(ipv4_dst=self.rt_surrogate_origin[ip_dst][udp_dst]['udp']['surrogate']),parser.OFPActionOutput(out_port)]
					else:
						if (dpid in self.mac_ce_borders):
							if (self.mac_ce_borders[dpid] == mac_src):
					...
					if (out_port != ofproto.OFPP_FLOOD):
						#agrego el flujo
						if (len(self.origins[ip_dst][udp_dst]['udp']) > 0):
							del self.rt_surrogate_origin[ip_dst][udp_dst]['udp']
				                	self.add_flow(datapath, 100, match, actions, 30)
				elif ((ip_src in self.surrogates) and (udp_src in self.surrogates[ip_src]) and ('udp' in self.surrogates[ip_src][udp_src])):
					# construct action list.
					match = parser.OFPMatch(eth_type=ether.ETH_TYPE_IP,udp_src=udp_src,ip_proto=ipv4_pkt.proto,ipv4_src=ip_src)
					...
					if (out_port != ofproto.OFPP_FLOOD):
						#agrego el flujo
				                self.add_flow(datapath, 100, match, actions, 30)
				elif ((ip_dst in self.surrogates and udp_dst in self.surrogates[ip_dst] and 'udp' in self.surrogates[ip_dst][udp_dst]) or (ip_dst in self.origins and udp_dst in self.origins[ip_dst] and 'udp' in self.origins[ip_dst][udp_dst])):
					match = parser.OFPMatch(eth_type=ether.ETH_TYPE_IP,udp_dst=udp_dst,ip_proto=ipv4_pkt.proto,ipv4_dst=ip_dst,ipv4_src=ip_src)
					if (dpid in self.mac_ce_borders):
						if (self.mac_ce_borders[dpid] == mac_src):
							actions = [parser.OFPActionOutput(out_port)]
						else:			
					...
					if (out_port != ofproto.OFPP_FLOOD):
						print "agrego el flujo"
					        #self.add_flow(datapath, 100, match, actions)
				elif ((ip_src in self.surrogates and udp_src in self.surrogates[ip_src] and 'udp' in self.surrogates[ip_src][udp_src]) or (ip_src in self.origins and udp_src in self.origins[ip_src] and 'udp' in self.origins[ip_src][udp_src])):
					match = parser.OFPMatch(eth_type=ether.ETH_TYPE_IP,udp_src=udp_src,ip_proto=ipv4_pkt.proto,ipv4_dst=ip_dst,ipv4_src=ip_src)
					if (dpid in self.mac_ce_borders):
						if (self.mac_ce_borders[dpid] == mac_src):
							actions = [parser.OFPActionOutput(out_port)]
						else:			
...
class GUIServerController(ControllerBase):
	...
	if ((int(portToadd) not in self.mainapp.all_tcpports) or (transportToadd not in self.mainapp.all_tcpports[int(portToadd)])):
		...
		arreglo = r.json()
		for switch in arreglo:
			if (transportToadd == "tcp"):
				r = requests.post('http://localhost:8080/stats/flowentry/add', data = json.dumps({"dpid":int(switch),"table_id":0,"priority":1,"match":{"eth_type":ether.ETH_TYPE_IP,"tcp_dst":int(portToadd),"ip_proto":6},"actions":[{"type":"OUTPUT","port":ofproto_v1_3.OFPP_CONTROLLER}]}))
				r = requests.post('http://localhost:8080/stats/flowentry/add', data = json.dumps({"dpid":int(switch),"table_id":0,"priority":1,"match":{"eth_type":ether.ETH_TYPE_IP,"tcp_src":int(portToadd),"ip_proto":6},"actions":[{"type":"OUTPUT","port":ofproto_v1_3.OFPP_CONTROLLER}]}))
			else:
	...
	if ((ipOrigin in self.mainapp.origins) and (int(portOrigin) in self.mainapp.origins[ipOrigin]) and (transport in self.mainapp.origins[ipOrigin][int(portOrigin)])):
		...
		if ((int(portSurrogate) not in self.mainapp.all_tcpports) or (transport not in self.mainapp.all_tcpports[int(portSurrogate)])):
			...
			for switch in arreglo:
				if (transport == "tcp"):
					r = requests.post('http://localhost:8080/stats/flowentry/add', data = json.dumps({"dpid":int(switch),"table_id":0,"priority":1,"match":{"eth_type":ether.ETH_TYPE_IP,"tcp_dst":int(portSurrogate),"ip_proto":6},"actions":[{"type":"OUTPUT","port":ofproto_v1_3.OFPP_CONTROLLER}]}))
					r = requests.post('http://localhost:8080/stats/flowentry/add', data = json.dumps({"dpid":int(switch),"table_id":0,"priority":1,"match":{"eth_type":ether.ETH_TYPE_IP,"tcp_src":int(portSurrogate),"ip_proto":6},"actions":[{"type":"OUTPUT","port":ofproto_v1_3.OFPP_CONTROLLER}]}))
				else:
					r = requests.post('http://localhost:8080/stats/flowentry/add', data = json.dumps({"dpid":int(switch),"table_id":0,"priority":1,"match":{"eth_type":ether.ETH_TYPE_IP,"udp_dst":int(portSurrogate),"ip_proto":17},"actions":[{"type":"OUTPUT","port":ofproto_v1_3.OFPP_CONTROLLER}]}))
